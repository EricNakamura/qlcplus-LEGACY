...
AddFixture::AddFixture(QWidget *parent, const Doc *doc, const Fixture *fxi)
  ...
  connect(m_modeCombo, SIGNAL(activated(int)), this,
          SLOT(slotModeActivated(int)));
  connect(m_universeCombo, SIGNAL(activated(int)), this,
          SLOT(slotUniverseActivated(int)));
  connect(m_addressSpin, SIGNAL(valueChanged(int)), this,
          SLOT(slotAddressChanged(int)));
  connect(m_channelsSpin, SIGNAL(valueChanged(int)), this,
          SLOT(slotChannelsChanged(int)));
  connect(m_nameEdit, SIGNAL(textEdited(const QString &)), this,
          SLOT(slotNameEdited(const QString &)));
  connect(m_gapSpin, SIGNAL(valueChanged(int)), this,
          SLOT(slotGapSpinChanged(int)));
  connect(m_amountSpin, SIGNAL(valueChanged(int)), this,
          SLOT(slotAmountSpinChanged(int)));
  connect(m_searchEdit, SIGNAL(textChanged(QString)), this,
          SLOT(slotSearchFilterChanged(QString)));
  connect(m_diptoolButton, SIGNAL(clicked()), this,
          SLOT(slotDiptoolButtonClicked()));
  connect(m_buttonCreateFix, SIGNAL(clicked()), this,
          SLOT(slotCreateFixtureClicked()));
...
QLCFixtureDef *AddFixture::fixtureDef() const { return m_fixtureDef; }

QLCFixtureMode *AddFixture::mode() const { return m_mode; }

QString AddFixture::name() const { return m_nameValue; }

quint32 AddFixture::address() const { return m_addressValue; }

quint32 AddFixture::universe() const { return m_universeValue; }

int AddFixture::amount() const { return m_amountValue; }

quint32 AddFixture::gap() const { return m_gapValue; }

quint32 AddFixture::channels() const { return m_channelsValue; }

bool AddFixture::invalidAddress() const { return m_invalidAddressFlag; }
...
void AddFixture::fillTree(const QString &selectManufacturer,
  ...
  QStringListIterator modit(
      m_doc->fixtureDefCache()->models(KXMLFixtureGeneric));
  while (modit.hasNext() == true) {
    model = modit.next();
    child = new QTreeWidgetItem(parent);
    child->setText(KColumnName, model);
...
void AddFixture::findAddress() {
  ...
  quint32 address =
      findAddress((m_channelsValue + m_gapValue) * m_amountValue,
                  m_doc->fixtures(), m_doc->inputOutputMap()->universesCount());
...
void AddFixture::slotCreateFixtureClicked() {
...
void AddFixture::slotModeActivated(int modeIndex) {
  ...
  for (int i = 0; i < m_mode->channels().size(); i++) {
    ...
    new QListWidgetItem(QString("%1: %2").arg(i + 1).arg(channel->name()),
                        m_channelList);
  }
}
...
void AddFixture::slotUniverseActivated(int universe) {
  ...
  quint32 addr = findAddress(universe, m_channelsSpin->value(),
                             m_doc->fixtures(), m_fixtureID);
...
void AddFixture::slotSelectionChanged() {
  ...
  if (manuf == KXMLFixtureGeneric && model == KXMLFixtureGeneric) {
    ...
    if (m_fixtureID != Fixture::invalidId()) {
      ...
      if (fxi != NULL) {
        ...
        if (m_fixtureDef->manufacturer() != manuf ||
            m_fixtureDef->model() != model) {
          m_fixtureDef = NULL;
        }
      } else {
        m_fixtureDef = NULL;
  ...
  quint32 addr = findAddress(m_universeValue, m_channelsSpin->value(),
                             m_doc->fixtures(), m_fixtureID);
  if (addr != QLCChannel::invalid())
    m_addressSpin->setValue((addr & 0x01FF) + 1);
  else
